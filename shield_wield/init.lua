...
local ATTACH_DEFAULTS = {
    ...
    block_pos = {x = 1.300, y = 6.350, z = -1.100},
    block_rot = {x = 102.700, y = -1.500, z = -15.000},
...
local ARM_POSE_DEFAULTS = {
    ...
    block_pos = {x = -0.350, y = 0.350, z = 0.550},
    block_rot = {x = -105.000, y = 18.000, z = -2.000},
}
...
SW.PERSIST_TUNING = minetest.settings:get_bool("shield_wield_persist_tuning", false)

local stored_attach = nil
local stored_arm = nil
if SW.PERSIST_TUNING then
    stored_attach = minetest.deserialize(storage:get_string("attach_pose"))
    stored_arm = minetest.deserialize(storage:get_string("arm_pose"))
else
    -- Ignore stale tuning values when persistence is disabled so bad poses do not stick.
    storage:set_string("attach_pose", "")
    storage:set_string("arm_pose", "")
end

SW.attach = normalize_attach_pose(stored_attach)
SW.arm_pose = normalize_arm_pose(stored_arm)
...
local function apply_arm_pose(player, raised)
    ...
    local rotation_absolute = raised and true or false
